...
The code example below shows how to delete VBA module with name "Module1".
...
```sh
curl -X DELETE "https://api.aspose.cloud/v3.0/slides/MyPresentation.pptx/vbaProject/Module1" -H "Authorization: Bearer [Access Token]" -H "Content-Type: text/json"
```
...
VbaProject response = api.DeleteVbaModule("MyPresentation.pptx", "Module1");
...
```java
SlidesApi api = new SlidesApi("MyClientId", "MyClientSecret");
VbaProject response = (VbaProject)api.deleteVbaModule("MyPresentation.pptx", "Module1", null, null, null);
...
$result = $api->deleteVbaModule("MyPresentation.pptx", "Module1");
...
response = api.delete_vba_module("MyPresentation.pptx", "Module1")
...
const response = await api.deleteVbaModule("MyPresentation.pptx", "Module1").then((response) => {
...
response, _, e := api.DeleteVbaModule(fileName, "Module1", "", "", "")
if e != nil {
    t.Errorf("Error: %v.", e)
    return
}
...
my %params = ('name' => 'MyPresentation.pptx', 'moduleName' => 'Module1');
